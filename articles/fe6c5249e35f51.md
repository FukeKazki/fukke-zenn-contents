---
title: "オフィスのCO2濃度を計測して、集中時間を増やす"
emoji: "📑"
...
室内のCO2濃度は作業効率に影響を与えると言われています。
日常的にCO2を意識して換気をしている人は多くありません。

空気中のCO2濃度は概ね410ppm程度とされており、室内のCO2濃度は1000ppm以下が基準と言われています。
部屋のCO2濃度が高くなると息苦しさや眠気などの症状がでてきます。

今回は、作業部屋のCO2濃度を可視化し、作業へ集中できるようにした取り組みを紹介します。
CO2濃度の上昇を通知し適切なタイミングで換気をすることで、集中を持続できます。

## 全体構成

CO2センサーにはSwitchBotの製品を利用しました。
またSwitchBot APIからCO2センサーの値を取得するためにハブミニも利用しました。

[SwitchBot CO2 センサー｜空気管理をスマートに – SwitchBot (スイッチボット)](https://www.switchbot.jp/products/switchbot-co2-meter)
![CO2センサー](https://storage.googleapis.com/zenn-user-upload/3dcd77a150df-20260127.jpeg)

[SwitchBot ハブミニ｜リモコンを一つにまとめるスマートリモコン – SwitchBot (スイッチボット)](https://www.switchbot.jp/products/switchbot-hub-mini)
![壁面へ設置したハブミニ](https://storage.googleapis.com/zenn-user-upload/7f9ed3589658-20260127.jpeg)

通知先はSlackにしました。
筆者のPC(Mac)上でCRONスクリプトを実行し定期的にCO2濃度を取得、通知します。

```mermaid
flowchart LR
  Sensor[CO2センサー] -- 計測値 --> API[SwitchBot API]
  API -- 取得 --> Cron[CRONスクリプト]
  Cron -- 通知 --> Slack[Slack]
```
...
1. SwitchBotアプリを開き、「プロフィール」→「設定」に移動
2. 「アプリのバージョン」を10回連続でタップ
3. 「開発者向けオプション」が表示される
4. 「トークン」と「クライアントシークレット」を取得
...
結果から気温24.4℃、湿度24%、CO2濃度646ppmであることがわかります。
...
次に取得したセンサーの値をSlackに通知する部分を作成します。
Slack Botを作成してIncoming Webhookを設定し通知します。
...
![Slack通知](https://storage.googleapis.com/zenn-user-upload/de3b9a72ce4f-20260127.png)
...
これによってMacをスリープモードにしているときも動作するようになりました。加えて、PCの起動ごとにスクリプトを実行しにいく手間もなくなりました。

下記のような設定ファイルを準備し,`pm2 start ecosystem.config.cjs`で実行します。
...
またNodeの`experimental-strip-types`を利用してTypeScriptを直接実行できるようにしています。

## まとめ

この仕組みを作ったことで、CO2濃度の上昇を検知して早めに換気をし集中を持続させる仕組みをつくれた。
仕事のしやすい環境作りの意識が持てた。

1人が部屋で仕事をしているときは660ppm程度ですが、2人で1時間ほど作業すると1000ppmを超えることがわかりました。

湿度の低さが気になり、加湿器の必要性にも気づけました。
ぜひ取り組んでみてください。
