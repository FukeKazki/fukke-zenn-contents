...
## 概要
- `vp create`でライブラリのテンプレートを作成する
- vitest・oxlint・oxfmt・tsdownなどが最初から揃ったモダンな構成が手に入る
- 実装からpnpmを使ってnpmに公開するまでの一連の流れがわかる
...
これまでライブラリを一から作る際は、バンドラーの選定、`tsconfig.json`の設定、`package.json`の`exports`フィールドの調整、vitestなどのテスト環境の導入、Linter・Formatterの設定など、実装そのものとは関係のない部分で悩んだり時間を取られたりすることが多くありました。Vite+はこうした準備を統合ツールチェーンとしてまとめて提供してくれるため、`vp create`を実行するだけでモダンな開発環境がすぐに整い、実装に集中できる点に大きなメリットを感じました。
