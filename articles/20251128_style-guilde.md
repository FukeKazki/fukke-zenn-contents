...
プロンプトで「キャメルケースを使用する」「インポート順序を整理する」といった指示を毎回与えるのは効率的でないです。
...
ここでのルールの選定については、また別の記事で詳しく解説したいます。
...
フォーマットルール（インデント幅やセミコロン有無など）を共通化し、どのリポジトリでも同じ整形結果を指定しています。 
...
これらを組み合わせることで、人間とAIの両方が迷わずに、かつ高品質なコードを書き続けられる環境が整います。
これらの具体的な設定やCoding Agentとの連携ノウハウについては、また別の記事で解説します。
